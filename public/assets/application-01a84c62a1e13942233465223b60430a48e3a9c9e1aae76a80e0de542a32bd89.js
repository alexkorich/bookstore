...
(function() {
  $(document).on("ajax:success", "form.book", function(e, data, status, xhr) {
    if (data.price != null) {
...
(function() {
  ...
  $(document).on("ajax:success", "a", function(e, data, status, xhr) {
    ...
    $(".total-price").html("$" + data.price.toFixed(2) + "(" + data.quantity + ")");
    return $(".total-price-table").html("$" + data.price.toFixed(2));
  });
...
(function() {
  $(document).ready(function() {
    ...
    $("#useExistingCard_").click(function() {
      var isDisabled;
      isDisabled = $("#useExistingCard_").prop('checked');
      console.log(isDisabled);
      if (!isDisabled) {
        $("form.card-creation input, select").removeAttr("disabled");
        return $("form.card-selection input.btn,form.card-selection input:radio").prop('disabled', true);
      } else {
        $("form.card-creation input, select").prop('disabled', true);
        return $("form.card-selection input.btn,form.card-selection input:radio").removeAttr("disabled");
      }
    });
    isDisabled = $("#useExistingCard_").prop('checked');
    if (!isDisabled) {
    ...
    $("#useBilling_").click(function() {
      var isHide;
      isHide = $("#useBilling_").prop('checked');
      console.log(isHide);
      if (!isHide) {
        return $("ul.shipping").show();
      } else {
        return $("ul.shipping").hide();
      }
    });
    isHide = $("#useBilling_").prop('checked');
    if (isHide) {
...
(function() {
  $(document).on("ajax:success", "form.rate-form", function(e, data, status, xhr) {
    if (data) {
      $("div.rating").prepend("<div>" + data.author + " " + "<div id=stars>s</div>" + " " + data.text_review + "</div>").each(function() {
        return $(this).find("#stars").raty({
          number: 10,
          starOff: "https://s3.amazonaws.com/bookstore-korich-assets/assets/star-off.png",
          starOn: "https://s3.amazonaws.com/bookstore-korich-assets/assets/star-on.png"
        }).raty('score', data.rating);
      });
      return false;
    } else {
      return false;
